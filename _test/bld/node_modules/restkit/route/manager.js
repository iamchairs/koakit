...
;
class RouteManager {
    static runRoute(route, ctx) {
        return new Promise((resolve) => {
            let rules = reflect_1.Reflect.getMetadata('Rules', route.object, route.key) || [];
            rule_1.RuleService.runRules(rules, ctx).then(() => {
                return injector_1.InjectorService.run(route.object, route.key, ctx);
            }).then((response) => {
                let responseType = reflect_1.Reflect.getMetadata('ResponseType', route.object, route.key);
                if (responseType) {
    ...
    static bindStaticFiles(server, staticPaths) {
        staticPaths.forEach((p) => {
            server.baseRouter.get(p.uri, (req, res) => {
